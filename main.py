...
def rf_prediction(sequence, rf_model, win_loss_history):
    # Convert win/loss history ('Win'/'Loss') to numerical values (1/0)
    win_loss_history = [1 if result == 'Win' else 0 for result in win_loss_history]

    # Ensure win_loss_history is long enough to match sequence
    if len(win_loss_history) < len(sequence):
        win_loss_history = [0] * (len(sequence) - len(win_loss_history)) + win_loss_history  # Padding with 0 (Loss)

    # Use win/loss history as additional features
    features = np.array([[sequence[-1], win_loss_history[-1]]])  # last number and last result
    prediction = rf_model.predict(features)
    return "Big" if prediction[0] > 5 else "Small"

# LSTM Model Prediction
def lstm_prediction(sequence, lstm_model, win_loss_history):
    # Ensure win_loss_history is long enough to match sequence
    if len(win_loss_history) < len(sequence):
        win_loss_history = [0] * (len(sequence) - len(win_loss_history)) + win_loss_history  # Padding with 0 (Loss)

    # Reshape sequence and include win_loss_history in the model
    sequence = np.array(sequence).reshape((len(sequence), 1, 1))  # Reshape for LSTM input
    win_loss_history = np.array(win_loss_history).reshape((len(win_loss_history), 1, 1))
    prediction = lstm_model.predict(sequence[-1:])
    return "Big" if prediction[0] > 5 else "Small"

# Train Random Forest Model
def train_rf_model(sequence, win_loss_history):
    ...
    # Features: previous numbers and win/loss history as features
    features = np.array([[sequence[i], win_loss_history[i]] for i in range(len(sequence) - 1)])
    labels = np.array(sequence[1:])
    rf_model.fit(features, labels)
    return rf_model

# Train LSTM Model
def train_lstm_model(sequence, win_loss_history):
...
async def handle_message(update: Update, context):
    ...
    if text.isdigit():
        ...
        # Train models
        rf_model = train_rf_model(sequence, win_loss_history)
        lstm_model = train_lstm_model(sequence, win_loss_history)

        # Get predictions from ARIMA, Random Forest, and LSTM models
        arima_pred = arima_prediction(sequence)
        rf_pred = rf_prediction(sequence, rf_model, win_loss_history)
        lstm_pred = lstm_prediction(sequence, lstm_model, win_loss_history)
        ...
        prediction = "Big" if sum(user_data[user_id]['sequence']) > len(user_data[user_id]['sequence']) * 4 else "Small"
        final_prediction = generate_final_prediction([prediction], user_id)
        await update.message.reply_text(final_prediction)
        await update.message.reply_text("Send the next sequence for analysis.")

# Start the bot
if __name__ == '__main__':
    application = Application.builder().token('7638208028:AAHchjtDECdiyEaJNCyVvvoxsyvzaDr4Vek').build()
